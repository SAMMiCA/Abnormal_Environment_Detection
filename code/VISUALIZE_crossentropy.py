...
resnet50.load_state_dict(torch.load(f'./crossentropy_weight/maximum_clustering.pth'))
device = 'cuda'
...
with torch.no_grad():
    for k, (x, y) in enumerate(test_loader):
        ...
        y = y.to(device)
        label.append(y)
        resnet50.avgpool.register_forward_hook(get_activation('avgpool'))

        output = resnet50(x)

        # result.append(torch.nn.functional.normalize(activation['avgpool']).squeeze())
...
plt.tight_layout()
plt.savefig('./pictures/crossentropy_GT')
plt.show()

#kmeans

kmeans = KMeans(n_clusters=4, random_state=0).fit(features)
# dbscan = DBSCAN(eps=3).fit(transformed)
...
plt.tight_layout()
plt.savefig('./pictures/crossentropy_KMeans')
plt.show()
...
(convert_binary(labels,0),convert_binary(remapped_labels,0))
binary_recall_normal = recall_score(convert_binary(labels,0),convert_binary(remapped_labels,0)) * 100
