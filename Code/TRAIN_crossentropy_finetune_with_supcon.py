...
train_dataset = ImageFolder(root='../Dataset/train_few/',transform=transform)
...
criterion = losses.SupConLoss(temperature=1)
device = 'cuda'
...
for epoch in range(1):
    ...
    if score_c < val_clustering_score:
        torch.save(resnet50.state_dict(), os.path.join('./crossentropy_weight/', 'maximum_clustering_finetuning2.pth'))
        score_c = val_clustering_score
        if score_c == 1.0:
            break
    if loss_v > val_results['loss_v']:
        torch.save(resnet50.state_dict(), os.path.join('./crossentropy_weight/', 'minimum_loss_finetuning2.pth'))
